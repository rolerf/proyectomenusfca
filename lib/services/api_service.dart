...
class ApiService {
  ...
  static Future<Map<String, dynamic>> verificarToken(
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/verificar_token.php"),
      headers: {"Content-Type": "application/json"},
      body: jsonEncode({"correo": correo, "token": token}),
    );

    if (response.statusCode == 200) {
      return jsonDecode(response.body);
    } else {
      return {"status": "error", "mensaje": "Error al verificar token"};
    }
  }

  static Future<DateTime?> obtenerHoraServidor() async {
    try {
      final url = Uri.parse("$baseUrl/get_server_time.php");
      final res = await http.get(url).timeout(Duration(seconds: 6));
